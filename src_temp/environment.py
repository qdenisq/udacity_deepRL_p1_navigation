...
class BananaEnvironment:
    def __init__(self, file_name=None, **kwargs):
        self.__env = UnityEnvironment(file_name=file_name, no_graphics=True)  # create environment
        self.__brain_name = self.__env.brain_names[0]
        self.__env.reset()
        self.__state_dim = np.array(self.__env.brains[self.__brain_name].vector_observation_space_size)
...
class VisualBananaEnvironment:
    ...
    def step(self, action):
        env_info = self.__env.step(action)[self.__brain_name]  # step
        next_obs = env_info.visual_observations[0].squeeze().transpose()  # get the next state
        self.__cur_state = np.roll(self.__cur_state, -1, axis=1)
        self.__cur_state[:, -1, :, :] = next_obs
        reward = env_info.rewards[0]  # get the reward
        done = env_info.local_done[0]  # see if episode has finished
    ...
    def reset(self, train_mode=True):
        env_info = self.__env.reset(train_mode=train_mode)[self.__brain_name]
        obs = env_info.visual_observations[0].squeeze().transpose()
        self.__cur_state = np.stack([obs] * 4, axis=1)
